...
Composes the SAM segmentation loss with optional BNDL KL and AUE attacker
losses. The ``weight_schedule`` field implements the paper's curriculum
(clean -> +BNDL -> +AUE) by swapping the per-component weights at configured
epoch boundaries.
"""
...
class CombinedSAMBNDLLoss(nn.Module):
    """
    Combined loss for SAM training with BNDL and AUE.

    Components:
    - sam_loss: Segmentation task loss (focal + dice + iou)
    - bndl_loss: BNDL KL divergence loss
    - aue_loss: Adversarial task/BNDL/calibration loss on adv samples (attacker via GRL)
    """
