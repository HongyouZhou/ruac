...
"""BNDL KL divergence loss.

Computes the KL divergence between the variational Weibull posterior and a
Gamma prior, plus an optional UR-style uncertainty regularizer that ties
error magnitude to uncertainty so high-uncertainty regions keep contributing
gradient.
"""
...
class BNDLLoss(nn.Module):
    ...
    def forward(self, outs_batch: list[dict], targets_batch: torch.Tensor):
        ...
        Args:
            outs_batch: SAM outputs (carrying ``multistep_aux_outputs`` with a
                ``bndl`` namespace per step).
            targets_batch: targets, not used here (only consulted for device).
        """
        ...
        for outs in outs_batch:
            # Extract BNDL namespace from the unified aux_outputs dict.
            if "multistep_aux_outputs" in outs:
                ...
                # Backward-compat with the older flat key name.
    ...
    def _compute_kl_loss(self, bndl_outputs):
        ...
        # Pixel-level KL divergence (local sparsity).
        ...
        # Prompt-level KL divergence (global sparsity via hyper_in).
        ...
        # Pixel KL stays dominant; prompt KL is auxiliary regularization.
