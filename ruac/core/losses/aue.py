...
Computes losses on adversarial samples (task loss, BNDL KL, L_cal_sym
calibration). All terms enter the single ``core_loss``; backward through GRL
in Style/Deform networks flips the sign for attacker params, giving joint
min-max in one pass.
"""
...
from ruac.core.losses import CORE_LOSS_KEY


def _prepare_gt_for_loss(pixel_gt: torch.Tensor, target_size: tuple[int, int]) -> torch.Tensor:
    """Combine multi-object GT and resize to target spatial resolution."""
    if pixel_gt.shape[1] > 1:
        pixel_gt_combined = pixel_gt.sum(dim=1, keepdim=True).clamp(0, 1)
    else:
        pixel_gt_combined = pixel_gt
    return F.interpolate(pixel_gt_combined.float(), size=target_size, mode="nearest").squeeze(1)
...
class AUELoss(nn.Module):
    ...
    def __init__(
        ...
        cal_weight: float = 0.0,  # lambda from paper Eq. 5; coefficient on L_cal_sym in core_loss
        ...
        Args:
            ...
            cal_weight: Weight lambda on L_cal_sym (paper Eq. 5); updates decoder/BNDL via L1/L2 paths, attacker via GRL
    ...
    def _compute_adv_task_loss(self, adv_out: dict, device: torch.device) -> tuple[torch.Tensor, dict] | None:
        ...
        if multistep_masks is not None and multistep_ious is not None and multistep_obj_scores is not None:
            ...
            for src_masks, ious, object_score_logits in zip(multistep_masks, multistep_ious, multistep_obj_scores, strict=False):
    ...
    def _compute_calibration_loss(self, bndl_ns: dict, device: torch.device) -> torch.Tensor | None:
        ...
        Motivation: original Eq. 5 has d L/d e = exp(-u) - exp(u) ~= 0 at u ~= 0, so the
        attacker (via GRL) has no gradient signal in the confident region and cannot
        drive the confident-wrong failure mode. L2 restores a d/d u channel via the
        analytic uncertainty, symmetrically covering both UC and CW failure modes.

        === JOINT MIN-MAX (paper Eq. 7) ===
        Added to core_loss with weight ``cal_weight`` (= lambda). Single backward pass:
        ...
        # === Get pixel_logits (keep gradients for attacker and decoder) ===
        ...
        pixel_gt_resized = _prepare_gt_for_loss(pixel_gt_for_prep, (H_feat, W_feat))
        ...
        def _reduce_and_resize(u, target_shape):
            ...
            if u.shape[1:] != target_shape:
                u = torch.nn.functional.interpolate(u.unsqueeze(1), size=target_shape, mode="bilinear", align_corners=False).squeeze(1)
            return u
        ...
        # Drives d/d u channel: at confident-wrong (e=1, u~=0), d L2/d u = -1, so attacker
        ...
        if u_analy is not None:
            ...
            assert u_analy.shape == u_sample.shape, f"L2: u shape mismatch analytic={u_analy.shape} vs sampling={u_sample.shape}"
            e_sg = e_clamp.detach()
