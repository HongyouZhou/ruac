...
class AdversarialStyleGCN(nn.Module):
    ...
    def forward(self, style_deltas: torch.Tensor, edge_index: torch.Tensor, edge_weight: torch.Tensor, mask_features: torch.Tensor | None = None) -> torch.Tensor:
        ...
        for layer_idx, (linear, norm) in enumerate(zip(self.layers, self.layer_norms, strict=False)):
            x = linear(x)
    ...
    def _add_self_loops(self, edge_index: torch.Tensor, edge_weight: torch.Tensor, num_nodes: int) -> tuple[torch.Tensor, torch.Tensor]:
